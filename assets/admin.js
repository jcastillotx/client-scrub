...
function refreshStats() {
    jQuery.post(brm_ajax.ajax_url, {
        ...
        if (response.success) {
            var s = response.data || {};
            jQuery('.brm-stat-number[data-key="total_clients"]').text(s.total_clients != null ? s.total_clients : 0);
            jQuery('.brm-stat-number[data-key="total_results"]').text(s.total_results != null ? s.total_results : 0);
            jQuery('.brm-stat-number[data-key="recent_results"]').text(s.recent_results != null ? s.recent_results : 0);
        }
    });
}
